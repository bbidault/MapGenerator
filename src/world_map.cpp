...
///
void world_map::river( int i, int j )
{
    ...
    for( int k = 0 ; k < 3 ; k++ )
    {
        for( int l = 0 ; l < 3 ; l++ )
        {
            if( sign( k ) == 1 && sign( l ) == 1 )
            {
                if( 1.41*world[i + k - 1][j + l - 1].altitude < lowest )
                {
    ...
    if( /*world[row][col].state != LAKE &&*/ lowest > 0)
    {
        if( row != i || col != j )
        {
...
void world_map::visit_lake( int i, int j, std::vector <pixel_coordinates> lake )
{
    ...
    if( world[i + 1][j].state == LAKE && !world[i + 1][j].visited )
    {
        new_lake.row = i + 1;
        new_lake.col = j;
        lake.push_back( new_lake );
        world[i + 1][j].visited = true;
    ...
    if( world[i][j + 1].state == LAKE && !world[i][j + 1].visited )
    {
        new_lake.row = i;
        new_lake.col = j + 1;
        lake.push_back( new_lake );
        world[i][j + 1].visited = true;
...
///
void world_map::lake( int i, int j )
{    
    ...
    while( building_lake )
    {
        ...
        for( int k = 0 ; k < current_lake.size() ; k++ )
        {
            ...
            if( world[row + 1][col].altitude < world[row][col].altitude && world[row + 1][col].state != LAKE )
            {
                world[row + 1][col].state = RIVER;
                this->river( row + 1, col );
                building_lake = false;
                break;
            }
            else if( world[row + 1][col].altitude < lowest && world[row + 1][col].state != LAKE )
            {
            ...
            if( world[row][col + 1].altitude < world[row][col].altitude && world[row][col + 1].state != LAKE )
            {
                world[row][col + 1].state = RIVER;
                this->river( row, col + 1 );
                building_lake = false;
                break;
            }
            else if( world[row][col + 1].altitude < lowest && world[row][col + 1].state != LAKE )
            {
